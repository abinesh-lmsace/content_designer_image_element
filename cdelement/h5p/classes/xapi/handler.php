...
use cdelement_h5p\event\statement_received;
use core_xapi\local\statement;
use core_xapi\handler as handler_base;
use core\event\base as event_base;
use core_xapi\local\state;
use cdelement_h5p\xapi\state_store;
 ...
 * @copyright  20224 bdecent gmbh <https://bdecent.de>
...
class handler extends handler_base {
    ...
    public function statement_to_event(statement $statement): ?event_base {
        ...
        if (!has_capability('mod/contentdesigner:view', $context, $user)) {
            return null;
        }

        $cm = get_coursemodule_from_id('contentdesigner', $context->instanceid, 0, false);
        if (!$cm) {
            return null;
        }
    ...
    /**
     * Retrieves the state store instance for the handler.
     *
     * @return state_store The state store associated with 'cdelement_h5p'.
     */
    public function get_state_store(): state_store {
        return new state_store('cdelement_h5p');
    }
}
